...
@Component({
  ...
  styleUrls: ['./ofertalaboral.component.scss']
})
export class OfertalaboralComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.oferta.idoferta == 0){
      this.router.navigate(["/profesional/ofertas"])      
    }
  ...
  postular(){
    if(sessionStorage.getItem("rutpro") != null){
      this.http.POSTSOL(this.oferta.idoferta, sessionStorage.getItem("rutpro")).subscribe(datos=>{
        if(datos.status == "ok"){
          this.mensaje = true;
          setTimeout(() => {
            this.router.navigateByUrl('/profesional/ofertas')
          }, 5000);  //5s
        }
      })
    }else{
      this.http.POSTSOL(this.oferta.idoferta, localStorage.getItem("rutpro")).subscribe(datos=>{
        if(datos.status == "ok"){
          this.mensaje = true;
          setTimeout(() => {
            this.router.navigateByUrl('/profesional/ofertas')
